...
function Card({ children, style = {} }) {
  return (
    <div style={{
      background: "rgba(12,10,26,0.75)", backdropFilter: "blur(12px)",
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 11, color: "var(--muted)" }}>CENTER</span>
        <span style={{ fontSize: 11, color: "#c084fc", letterSpacing: "0.05em" }}>RIGHT</span>
      </div>
  ...
  return (
    <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {["Text", "URL"].map((mode) => {
            ...
            return (
              <button key={mode} onClick={() => setIsUrl(mode === "URL")} style={{
                ...
                color: active ? "#c084fc" : "var(--muted)", cursor: "pointer", transition: "all 0.2s"
              }}>
                {mode}
              </button>
            )
      ...
      <button onClick={handleAnalyze} disabled={loading || !input.trim()} style={{
        ...
        background: loading ? "rgba(139,92,246,0.3)" : "linear-gradient(135deg, #16a34a, #ec4899, #2aaefa)",
...
function ResultsView({ data }) {
  if (!data) return (
    <div style={{ padding: "40px 16px", textAlign: "center" }}>
      <p style={{ color: "var(--muted)", fontSize: 14 }}>No results yet. Analyze an article first.</p>
    </div>
  )
  ...
  const scores = [
    ...
    },
    {
      ...
      colorFrom: "#a855f7", colorTo: "#ec4899"
    }
  ]
    ...
    <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 12, color: "#c084fc", fontWeight: 500 }}>{data.bias_label}</span>
        </div>
        ...
        <Card key={s.label}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 12, color: "var(--muted)" }}>{s.label2}</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text)" }}>{s.score}</span>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {data.key_claims.map((claim, i) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                ...
                <span style={{ fontSize: 12, color: "var(--text)", lineHeight: 1.6 }}>{claim}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      {history.map((item, i) => (
        <Card key={i} style={{ cursor: "pointer" }} onClick={() => onSelect(item)}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: "#c084fc" }}>{item.bias_label}</span>
            <span style={{ fontSize: 11, color: "var(--muted)" }}>Credibility: {item.credibility_score}</span>
          </div>
          <p style={{ fontSize: 12, color: "var(--text)", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
            {item.summary}
          </p>
        </Card>
      ))}
...
export default function App() {
  ...
  return (
    <div style={{ width: 360, minHeight: 580, background: "var(--bg)", position: "relative", display: "flex", flexDirection: "column" }}>
      {orbs}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            background: "linear-gradient(135deg, #16a34a, #ec4899, #2aaefa)",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 16, boxShadow: "0 4px 12px rgba(139,92,246,0.4)"
          }}>🔎</div>
          <div>
            <h1 style={{ fontSize: 15, fontWeight: 700, letterSpacing: "0.02em", lineHeight: 1 }}>TruthLens</h1>
            <p style={{ fontSize: 10, color: "var(--muted)", letterSpacing: "0.1em", marginTop: 2 }}>MEDIA ANALYSIS ENGINE</p>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        backdropFilter: "blur(16px)", background: "rgba(3,2,8,0.8)"
      }}>
        {navItems.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => setView(item.id)} style={{
              ...
              <span style={{ fontSize: 10, color: active ? "#c084fc" : "var(--muted)", fontWeight: active ? 600 : 400 }}>
                {item.label}
              </span>
            </button>
          )
